...
class DocumentService {
	...
	public async getDocumentById(
		...
		const docs = documents.find(
			(doc) => doc.id === id && doc.userId === userId,
		);
		if (!docs)
			throw new ApiErrors("Documento não encontrado.", StatusCodes.NOT_FOUND);
		return docs;
	}
	...
	public async updateDocument(
		...
		const index = documents.findIndex(
			(doc) =>
				doc.id === updatedDocument.id && doc.userId === updatedDocument.userId,
		);
		if (index === -1)
			throw new ApiErrors("Documento não encontrado.", StatusCodes.NOT_FOUND);
